...
do
	...
	mkdir -p ${i}/4resp-ind
	cat <<-EOF > ${i}/4resp-ind/py_resp.run
		#!/bin/csh -f
		echo " "
		...
		exit(1)
		EOF
	(cd ${i}/4resp-ind; rm ${i}.*; pyresp_gen.py -i ${espdata} -f1 ${i}.1st -f2 ${i}.2nd -p 'ind' -qwt1 0.00005 -qwt2 0.0001; chmod +x py_resp.run; ./py_resp.run)
